...
async function main() {
  ...
  for (const studentId of students) {
    for (const subject of testSubjects) {
        // Distribute grades across different semesters based on subject code
        const semesterIdFilter = subject.code.includes('-E1-') ? 
            (subject.code.includes('-SEM-1') ? 'E1-SEM-1' : 'E1-SEM-2') :
            (subject.code.includes('-SEM-1') ? 'E2-SEM-1' : 'E2-SEM-2');
        ...
        await prisma.grade.upsert({
            ...
            update: { grade: Math.floor(Math.random() * 6) + 5 }, // Random grade 5-10
            create: {
                studentId,
                subjectId: subject.id,
                semesterId: semesterIdFilter,
                grade: Math.floor(Math.random() * 6) + 5,
                batch: studentId.substring(0, 3)
            }
        });

        // 5. Seed Sample Attendance
        const totalClasses = Math.floor(Math.random() * 15) + 30; // 30-45 classes
        const attendedClasses = Math.floor(Math.random() * (totalClasses - 20)) + 20; // At least 20 attended
        ...
        await prisma.attendance.upsert({
            ...
            update: { totalClasses, attendedClasses, updatedAt: new Date() },
            create: {
                studentId,
                subjectId: subject.id,
                semesterId: semesterIdFilter,
